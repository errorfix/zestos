...
interface CheckInTerminalWrapperProps {
  initialAuthenticated: boolean;
  initialRoleLabel?: string;
}

export default function CheckInTerminalWrapper({
  initialAuthenticated,
  initialRoleLabel,
}: CheckInTerminalWrapperProps) {
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(initialAuthenticated);
  const [roleLabel, setRoleLabel] = useState<string>(initialRoleLabel || 'Gate Security Team');
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    if (!password.trim()) {
      setErrorMessage('Please enter the gate check-in password.');
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        if (res.ok && data.success) {
          ...
          setRoleLabel(data.user?.roleLabel || 'Gate Security & Check-In Team');
          setPassword('');
        } else {
          setErrorMessage(data.error || 'Invalid check-in password. Access denied.');
        }
      } catch (err) {
      ...
      <div className="max-w-md mx-auto my-8">
        <div className="bg-white rounded-3xl p-7 sm:p-9 border border-slate-200 shadow-xl text-center relative overflow-hidden">
          ...
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">
            Gate Terminal Locked
          </h2>
          <p className="text-xs text-slate-600 mt-2 leading-relaxed">
            Enter the designated Gate Check-In password to activate the camera barcode scanner and attendee verification console.
          </p>
          ...
          <form onSubmit={handleUnlock} className="mt-6 space-y-4 text-left">
            <div>
              ...
              >
                Gate Passcode / Password <span className="text-red-500">*</span>
              </label>
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  <span>Unlock Camera Terminal</span>
                </>
              )}
            </button>
          </form>
          ...
          <p className="text-[11px] text-slate-400 mt-5 border-t border-slate-100 pt-4">
            Confidential gate checkpoint • Lingaya&apos;s Vidyapeeth Fest Operations
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      {/* Authenticated Gate Banner with Lock Terminal Button */}

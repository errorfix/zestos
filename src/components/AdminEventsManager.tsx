...
export default function AdminEventsManager({
  ...
  const openEditModal = (evt: InitialEventData) => {
    ...
    setFormData({
      ...
      onSpotFeeInr: evt.onSpotFeeAmount != null ? evt.onSpotFeeAmount / 100 : '',
    ...
    startTransition(async () => {
      try {
        const payload = {
          ...
          onSpotFeeInr:
            formData.onSpotFeeInr !== '' && !isNaN(Number(formData.onSpotFeeInr))
              ? Number(formData.onSpotFeeInr)
              : null,
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Update online registration fee and on-spot desk walk-in price.
                </p>
              </div>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Cultural - Theatre">Cultural - Theatre</option>
                  <option value="Cultural - Fashion">Cultural - Fashion</option>
                  <option value="Literary">Literary</option>
                  <option value="Gaming">Gaming</option>
                  <option value="Informalz">Informalz</option>
                  <option value="Technical">Technical</option>
                  <option value="Management">Management</option>
                </select>
              </div>

              {/* Pricing Section: Online Fee vs On-Spot Walk-in Counter Fee */}
              <div className="p-4 rounded-2xl bg-amber-50/60 border border-amber-200/80 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-amber-800">
                    On-spot walk-in counter can have increased pricing
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm bg-white font-semibold"
                    />
                    <p className="text-[10px] text-slate-500 mt-1">Pre-registration website fee</p>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-amber-950 mb-1">
                      On-Spot Walk-In Counter Fee (₹)
                    </label>
                    <input
                      type="number"
                      min="0"
                      value={formData.onSpotFeeInr}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          onSpotFeeInr: e.target.value === '' ? '' : Number(e.target.value),
                        })
                      }
                      placeholder={`Defaults to ₹${formData.feeInr}`}
                      className="w-full px-3.5 py-2.5 rounded-xl border border-amber-300 text-sm bg-white font-extrabold text-amber-950 focus:border-amber-600 focus:ring-1 focus:ring-amber-500"
                    />
                    <p className="text-[10px] text-amber-800 mt-1">
                      Desk collection fee (e.g. ₹200 vs ₹150)
                    </p>
                  </div>
                </div>
              </div>
